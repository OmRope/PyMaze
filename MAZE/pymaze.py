...
class maze:
    ...
    def _drawMaze(self,theme):
        ...
        self._win.configure(bg="gray11")
        scr_width=900
        scr_height=650
        scr_height=scr_height-50
        self._win.geometry(f"{scr_width}x{scr_height}+0+0")
        self._canvas = Canvas(width=900, height=650, bg=theme.value[0],highlightthickness=0) # 0,0 is top left corner
        self._canvas.pack(expand=YES, side=RIGHT)
        window_width = self._win.winfo_width()
        window_height = self._win.winfo_height()
        pos_x = window_width - 600  # Position the canvas to the right
